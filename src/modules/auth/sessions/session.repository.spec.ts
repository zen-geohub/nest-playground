...
describe("SessionRepository", () => {
  ...
  describe("findToken", () => {
    it("should return token session record if active unrevoked token is found", async () => {
      const mockRow = {
        user_id: "user-123",
  ...
  describe("revokeTokenSession", () => {
    it("should set revoked_at timestamp for specified token session", async () => {
      dbService.query.mockResolvedValueOnce({
        rows: [],
        rowCount: 1,
        command: "UPDATE",
      ...
      await repository.revokeTokenSession("hashed_token_abc");
